...
public record BlurRenderer(
    ...
    private static final Supplier<SimpleFramebuffer> TEMP_FBO_SUPPLIER = Suppliers
            .memoize(() -> new SimpleFramebuffer("morbid_blur_temp", 1920, 1080, false));
    ...
    public void render(double x, double y, DrawContext ctx) {
        if (PipelineRegistry.BLUR_PIPELINE == null) return;

        final var client = MinecraftClient.getInstance();
        final var mainFbo = client.getFramebuffer();
        final var tempFbo = TEMP_FBO_SUPPLIER.get();

        if (mainFbo.getColorAttachmentView() == null) return;
        ...
        if (tempFbo.getColorAttachmentView() == null) return;

        Matrix3x2fStack guiMatrices = ctx.getMatrices();

        GameRendererAccessor gameRenderer = (GameRendererAccessor) client.gameRenderer;
        //MinecraftClient.getInstance().gameRenderer.guiRenderer.render(gameRenderer.getFogRenderer().getFogBuffer(FogRenderer.FogType.NONE));

  
        mainFbo.drawBlit(tempFbo.getColorAttachmentView());

        TextureSetup textureSetup = TextureSetup.of(tempFbo.getColorAttachmentView());

        SimpleGuiRenderState state = new SimpleGuiRenderState(
                PipelineRegistry.BLUR_PIPELINE,
                textureSetup,
                ctx,
